#include <stdio.h>
#include <stdlib.h>
#include "Arvore.h" // Inclui protótipos
...
// função recursiva que percorre a árvore pré-ordem imprimindo os dados.
...
// função recursiva que percorre a árvore pos-ordem imprimindo os dados.
...
{
	preOrdem_ArvAVLRecursive(raiz, 1);
};

static void preOrdem_ArvAVLRecursive(ArvAVL* raiz, int nivel){
    // condição de parada
   if (*raiz == NULL) return;
   // inicio da recorrência
   if (*raiz != NULL) {
   		print_tab_spaces(nivel);
   		printf("%d(%d)[%d]\n", nivel, (*raiz)->info, fatorBalanceamento_NO(*raiz));
    	preOrdem_ArvAVLRecursive(&((*raiz)->esq), nivel + 1);      
    	preOrdem_ArvAVLRecursive(&((*raiz)->dir), nivel + 1);
   }
   // fim da recorrência
};
...
{
	posOrdem_ArvAVLRecursive(raiz, 1);
};

static void posOrdem_ArvAVLRecursive(ArvAVL* raiz, int nivel){
    // condição de parada
   if (*raiz == NULL) return;
   // inicio da recorrência
   if (*raiz != NULL) {   				
    	posOrdem_ArvAVLRecursive(&((*raiz)->esq), nivel + 1);      
    	posOrdem_ArvAVLRecursive(&((*raiz)->dir), nivel + 1);
    	print_tab_spaces(nivel);   
    	printf("%d(%d)[%d]\n", nivel, (*raiz)->info, fatorBalanceamento_NO(*raiz));
   }
   // fim da recorrência
